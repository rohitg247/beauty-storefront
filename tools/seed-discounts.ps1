...
#   2. Automatic  - spend Rs2,499, get the travel-size Barrier Moisturiser free
#
...
$r = Send-GQL 'query { products(first: 30, query: "handle:travel-barrier-moisturiser") { nodes { id handle variants(first: 1) { nodes { id } } } } collections(first: 40, query: "handle:skincare") { nodes { id handle } } }'
$gift = $r.data.products.nodes | Select-Object -First 1
if (-not $gift) { Write-Host "FAIL - travel-barrier-moisturiser not found"; exit 1 }
...
# nine products, so it stands in for "anything in the catalogue".
